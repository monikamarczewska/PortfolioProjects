...
Select Location, MAX(cast(convert(float,Total_deaths) as int)) as TotalDeathCount
From PortfolioProject..CovidDeaths
Where continent is not null 
Group by Location
...
Select continent, MAX(cast(convert(float,Total_deaths) as int)) as TotalDeathCount
From PortfolioProject..CovidDeaths
Where continent is not null 
Group by continent
order by TotalDeathCount desc
...
DROP Table if exists #PercentPopulationVaccinated -- Drops table if already created, good for updating
